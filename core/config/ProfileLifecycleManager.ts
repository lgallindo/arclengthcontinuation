...
export class ProfileLifecycleManager {
  private savedConfigResult:
    ...
    | undefined;
  private savedBrowserConfigResult?: ConfigResult<BrowserSerializedArclengthContinuationConfig>;
  private pendingConfigPromise?: Promise<
    ConfigResult<ArclengthContinuationConfig>
  >;
  ...
  async loadConfig(
    ...
    this.pendingConfigPromise = new Promise((resolve) => {
      void (async () => {
        let result: ConfigResult<ArclengthContinuationConfig>;
  ...
  async getSerializedConfig(
    additionalContextProviders: IContextProvider[],
  ): Promise<ConfigResult<BrowserSerializedArclengthContinuationConfig>> {
