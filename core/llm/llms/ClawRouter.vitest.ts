...
describe("ClawRouter", () => {
  ...
  it("should include ArclengthContinuation User-Agent header", () => {
    const clawRouter = new ClawRouter({
      model: "blockrun/auto",
    });
    ...
    expect(headers["User-Agent"]).toMatch(/^ArclengthContinuation\//);
    expect(headers["X-ArclengthContinuation-Provider"]).toBe("clawrouter");
  });
