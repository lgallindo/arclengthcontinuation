...
import { ArclengthContinuationConfig, IdeInfo, IdeSettings } from "../index.js";
...
export class DataLogger {
  ...
  async addBaseValues(
    ...
    if ("userAgent" in zodSchema.shape) {
      newBody.userAgent = ideInfo
        ? `${ideInfo.name}/${ideInfo.version} (ArclengthContinuation/${ideInfo.extensionVersion})`
        : "Unknown/Unknown (ArclengthContinuation/Unknown)";
    }
  ...
  async logToOneDestination(
    dataConfig: NonNullable<ArclengthContinuationConfig["data"]>[number],
    event: DevDataLogEvent,
  ) {
    try {
      if (!dataConfig) {
