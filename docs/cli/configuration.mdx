...
`alc` resolves its configuration (models, MCP servers, rules, etc.) from several sources, in this order:
...
Point `alc` at a local YAML file:

```bash
alc --config ./my-config.yaml
```
...
`alc` looks for `~/.continue/config.yaml`. This file uses the same format as the IDE extensions — see [config.yaml reference](/customize/deep-dives/configuration) for the full schema.
...
alc --rule ./rules/style.md
alc --rule "Always use TypeScript strict mode"

# Load an agent file
alc --agent my-org/pr-reviewer
```
