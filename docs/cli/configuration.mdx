...
`crisfield` resolves its configuration (models, MCP servers, rules, etc.) from several sources, in this order:
...
Point `crisfield` at a local YAML file:

```bash
crisfield --config ./my-config.yaml
```
...
`crisfield` looks for `~/.continue/config.yaml`. This file uses the same format as the IDE extensions — see [config.yaml reference](/customize/deep-dives/configuration) for the full schema.
...
crisfield --rule ./rules/style.md
crisfield --rule "Always use TypeScript strict mode"

# Load an agent file
crisfield --agent my-org/pr-reviewer
```
