...
`arclen` resolves its configuration (models, MCP servers, rules, etc.) from several sources, in this order:
...
Point `arclen` at a local YAML file:

```bash
arclen --config ./my-config.yaml
```
...
`arclen` looks for `~/.continue/config.yaml`. This file uses the same format as the IDE extensions — see [config.yaml reference](/customize/deep-dives/configuration) for the full schema.
...
arclen --rule ./rules/style.md
arclen --rule "Always use TypeScript strict mode"

# Load an agent file
arclen --agent my-org/pr-reviewer
```
