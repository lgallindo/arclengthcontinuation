...
`cfld` resolves its configuration (models, MCP servers, rules, etc.) from several sources, in this order:
...
Point `cfld` at a local YAML file:

```bash
cfld --config ./my-config.yaml
```
...
`cfld` looks for `~/.continue/config.yaml`. This file uses the same format as the IDE extensions — see [config.yaml reference](/customize/deep-dives/configuration) for the full schema.
...
cfld --rule ./rules/style.md
cfld --rule "Always use TypeScript strict mode"

# Load an agent file
cfld --agent my-org/pr-reviewer
```
