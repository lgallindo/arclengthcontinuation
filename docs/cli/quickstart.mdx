...
Continue CLI (`crisfield`) is a terminal-based coding agent. It can edit files, run commands, and work through multi-step tasks — the same agent that powers the Continue IDE extensions, running in your terminal.

<Frame>
  <img src="/images/crisfield-demo.gif" />
</Frame>
...
This opens your browser to authenticate with [Continue](https://continue.dev). Once authenticated, `crisfield` can use your configured assistants, models, and MCP servers from the platform.
...
If you don't want to use Continue, you can use an Anthropic API key directly. On first launch, `crisfield` will prompt you to choose between logging in with Continue or entering an Anthropic API key.

## Two modes

**[TUI mode](/cli/tui-mode)** — run `crisfield` to start an interactive session. You type messages, reference files with `@`, approve tool calls, and iterate with the agent. This is the default.

**[Headless mode](/cli/headless-mode)** — run `crisfield -p "your prompt"` for single-shot automation. The agent runs to completion and prints its response to stdout. Use this in scripts, CI/CD, and git hooks.
...
Run `crisfield --help` for the full list.
