...
cfld --allow Bash --exclude "Bash(npm install*)"
```
...
```bash
cfld --auto       # Allow all tools without prompting
cfld --readonly   # Plan mode — read-only tools only, no file writes
```
