...
alc --allow Bash --exclude "Bash(npm install*)"
```
...
```bash
alc --auto       # Allow all tools without prompting
alc --readonly   # Plan mode — read-only tools only, no file writes
```
