...
arclen --allow "Write(**/*.ts)"

# Allow bash but not for install commands
arclen --allow Bash --exclude "Bash(npm install*)"
```
...
```bash
arclen --auto       # Allow all tools without prompting
arclen --readonly   # Plan mode — read-only tools only, no file writes
```
