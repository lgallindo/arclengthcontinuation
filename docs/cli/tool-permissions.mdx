...
crisfield --allow Write --allow Edit

# Exclude terminal commands
crisfield --exclude Bash
...
crisfield --allow "Write(**/*.ts)"

# Allow bash but not for install commands
crisfield --allow Bash --exclude "Bash(npm install*)"
```
...
```bash
crisfield --auto       # Allow all tools without prompting
crisfield --readonly   # Plan mode — read-only tools only, no file writes
```
