  ...
  <Tab title="⚡ Quick Start (Recommended)">
    ...
    <Steps>
      <Step title="Add the Pre-Built Agent">
        ...
        ```bash
        alc --agent continuedev/netlify-continuous-ai-agent
        ```
  ...
  performance:
    ...
    steps:
      ...
      - name: Authenticate Continue CLI
        ...
        run: |
          alc auth login --api-key "$CONTINUE_API_KEY"
      ...
      - name: Deploy and Test Performance
        ...
        run: |
          ...
          alc -p "Deploy PR preview and run Lighthouse.
