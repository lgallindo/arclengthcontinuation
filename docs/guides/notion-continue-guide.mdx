...
- [Continue CLI](https://lgallindo.github.io/) installed
...
<Steps>
  ...
  <Step title="Add Workspace Keys to Terminal Session">
    Depending on what you want `arclen` to accomplish, you'll need to add your Notion workspace keys to the terminal session. The workspace key is your Notion database ID. Run the following command:
...
<CardGroup cols={1}>
  <Card title="API Documentation" icon="code">
    ...
    **Headless Mode:**
    ```bash
    arclen -p --auto "Fetch my Notion databases using the API key and secrets stored in this session. Generate API docs 
