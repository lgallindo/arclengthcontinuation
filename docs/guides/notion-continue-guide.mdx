...
<Steps>
  ...
  <Step title="Add Workspace Keys to Terminal Session">
    Depending on what you want `alc` to accomplish, you'll need to add your Notion workspace keys to the terminal session. The workspace key is your Notion database ID. Run the following command:
...
<CardGroup cols={1}>
  <Card title="API Documentation" icon="code">
    ...
    **Headless Mode:**
    ```bash
    alc -p --auto "Fetch my Notion databases using the API key and secrets stored in this session. Generate API docs 
