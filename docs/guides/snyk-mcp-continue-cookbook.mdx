  ...
  <Tab title="⚡ Quick Start (Recommended)">
    <Steps>
      <Step title="Load the Pre-Built Agent">
        ...
        ```bash
        arclen --agent continuedev/snyk-continuous-ai-agent
        ```
      ...
      <Step title="Run Your First Security Scan">
        ...
        ```bash
         # Headless mode
        arclen -p "Run a complete security scan on this project including code vulnerabilities, dependencies, and any IaC files. Summarize findings by severity." --auto
        ```
...
<Steps>
  <Step title="Add Snyk MCP to Continue CLI">
    Add the Snyk MCP to your configuration:
    ...
    <Tip>
      **Installation methods:**
      1. **Quick CLI install**: `arclen --mcp snyk/snyk-mcp`
  ...
  <Step title="Add Secure-at-Inception Rules">
    Add security scanning rules to your configuration to enable automatic security scanning.
...
<Info>
  **Where to run these workflows:**
  ...
  - **CLI (headless mode)**: Use `arclen -p "your prompt" --auto` for headless commands
...
```bash
arclen -p "Run a Snyk Code scan on this repo with severity threshold medium. Summarize issues with file:line. Propose minimal diffs for the top 3 and rerun to verify." --auto
```
...
**Headless Mode Prompt:**
```bash
arclen -p "Run Snyk Open Source on this repo (include dev deps). Summarize vulnerable paths and propose a minimal-risk upgrade plan. Re-test after the plan (dry run)." --auto
```
...
**Headless Mode Prompt:**
```bash
arclen -p "Scan ./infra with Snyk IaC. Report high/critical misconfigs with exact files/lines. Provide code changes and re-scan to confirm." --auto
```
...
**Headless Mode Prompt:**
```bash
arclen -p "Scan image my-api:latest. Exclude base image vulns. Print dependency tree. Recommend a safer base image or upgrades. Re-test after the change (dry run)." --auto
```
...
**Headless Mode Prompt:**
```bash
arclen -p "Scan only files changed since origin/main with Snyk Code. Block if new high issues would be introduced. Show deltas." --auto
```
...
**Headless Mode Prompt:**
```bash
arclen -p "Open Snyk Learn lessons related to the top CWE(s) from this scan." --auto
```
      ...
      - name: Generate AI Mitigation Suggestions
        ...
        run: |
          ...
          if [ -n "$FINDINGS_SUMMARY" ]; then
            ...
            arclen --agent continuedev/snyk-continuous-ai-agent -p "$PROMPT" --auto > mitigation-suggestions.md || {
...
<Info>
  **About SNYK_TOKEN**: The workflow uses the SNYK_TOKEN in two ways:
  ...
  The `arclen` agent automatically uses the SNYK_TOKEN when needed for Snyk MCP operations.
</Info>
...
<Tip>
  Enable the **Secure-at-Inception** rules in your configuration to automatically apply
