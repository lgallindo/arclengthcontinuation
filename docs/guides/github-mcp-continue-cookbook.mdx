  ...
  <Tab title="⚡ Quick Start (Recommended)">
    <Steps>
      <Step title="Load the Pre-Built Agent">
        Run:
        ```bash
        arclen --agent continuedev/github-project-manager-agent
        ```
    ...
    <Info>
      You can also attach an MCP to a one-off session: `arclen --mcp anthropic/github-mcp`.
    </Info>
  </Tab>
...
<Info>
  **Where to run these workflows:**
  ...
  - **CLI (headless mode)**: Use `arclen -p "your prompt" --auto` for automation
  ...
  report:
    ...
    steps:
      ...
      - name: Post PR Context Comment
        ...
        run: |
          REPORT=$(        arclen --agent continuedev/github-project-manager-agent \
...
<CardGroup cols={2}>
  ...
  <Card title="GitHub Project Manager Agent" icon="robot" href="https://lgallindo.github.io/">
