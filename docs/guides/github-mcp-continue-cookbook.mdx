  ...
  <Tab title="⚡ Quick Start (Recommended)">
    <Steps>
      <Step title="Load the Pre-Built Agent">
        Run:
        ```bash
        alc --agent continuedev/github-project-manager-agent
        ```
    ...
    <Info>
      You can also attach an MCP to a one-off session: `alc --mcp anthropic/github-mcp`.
    </Info>
  </Tab>
...
<Info>
  **Where to run these workflows:**
  ...
  - **CLI (headless mode)**: Use `alc -p "your prompt" --auto` for automation
  ...
  report:
    ...
    steps:
      ...
      - name: Post PR Context Comment
        ...
        run: |
          REPORT=$(        alc --agent continuedev/github-project-manager-agent \
