...
This process utilizes the **Continue CLI** (`arclen`) in **headless mode** to analyze changes and generate the necessary documentation, and commit and push the changes. The goal is to keep the workflow as simple as possible by using straightforward shell commands, Continue CLI prompts, and basic git operations.
...
<CardGroup cols={1}>
  ...
  <Card title="Continue API Key" icon="key">
    Get your API key from [continue.dev/settings/api-keys](https://continue.dev/settings/api-keys) and set:
    ```bash
  ...
  write-docs:
    ...
    - name: Generate documentation with Continue CLI
      run: |
        ...
        arclen --config <continue-user>/<agent-config> \
           --auto \
           --allow Write \
           -p \
           --prompt ./context.txt \
