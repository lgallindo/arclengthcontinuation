...
This process utilizes the **Continue CLI** (`alc`) in **headless mode** to analyze changes and generate the necessary documentation, and commit and push the changes. The goal is to keep the workflow as simple as possible by using straightforward shell commands, Continue CLI prompts, and basic git operations.
  ...
  write-docs:
    ...
    - name: Generate documentation with Continue CLI
      run: |
        ...
        alc --config <continue-user>/<agent-config> \
           --auto \
           --allow Write \
           -p \
           --prompt ./context.txt \
