  ...
  <Tab title="⚡ Quick Start (Recommended)">
    <Steps>
      <Step title="Load the Pre-Built Agent">
        ...
        ```bash
        alc --agent continuedev/sanity-assistant-agent
        ```
  ...
  <Step title="Add Sanity MCP to Continue CLI">
    ...
    <Tip>
      **Installation methods:**
      1. **Quick CLI install**: `alc --mcp sanity/sanity-mcp`
...
<Info>
  **Where to run these workflows:**
  ...
  - **Terminal (TUI mode)**: Run `alc` to enter interactive mode, then type your prompts
  - **CLI (headless mode)**: Use `alc -p "your prompt"` for headless commands
  ...
  **Test in Plan Mode First**: Before running operations that might make
  ...
  To run any of the example prompts below in headless mode, use `alc -p "prompt"`
</Info>
  ...
  validate-content:
    ...
    steps:
      ...
      - name: Comment Report on PR
        ...
        run: |
          REPORT=$(          alc --agent continuedev/sanity-assistant-agent \
