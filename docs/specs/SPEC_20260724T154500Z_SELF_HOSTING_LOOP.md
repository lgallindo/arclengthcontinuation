# SPEC: self-hosting loop — arclen edits, rebuilds, and reinstalls arclen

## Intent

Prove the full self-hosting cycle on a host where arclen is installed from
source: headless arclen (agentic, tools auto-allowed) edits its own source
tree, the harness rebuilds the CLI from that edited source, runs unit
tests, reinstalls (in-place bundle rebuild behind the arclen binary —
`$ARCLEN_BIN`, default `command -v arclen` then `/usr/local/bin/arclen`; legacy
...
1. EDIT: arclen -p --auto, retried <=3 times, sets package.json "version" to
   `0.0.1-<marker>`; the harness verifies via git diff (edit made by arclen's
...
4. REINSTALL: rebuilt dist/arclen.js is executable behind the existing symlink.
5. VERIFY: `arclen --version` output contains `0.0.1-<marker>`.
