...
Make it possible to attest that a given `dist/cn.js` + `dist/index.js`
...
1. `scripts/attest-build.sh attest` on a clean tree with a fresh build
   ...
   package-lock.json, sha256 of dist/cn.js and dist/index.js, UTC
   timestamp.
...
4. Tampering with dist/cn.js after attest makes `verify` exit non-zero.
