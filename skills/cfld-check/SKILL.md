---
name: cfld-check
description: Install and run the Arclength-Continuation CLI (`cfld`) to execute AI agent checks on local code changes. Use when asked to "run checks", "lint with AI", "review my changes with cfld", or set up Arclength-Continuation CI locally.
...
# cfld check — Local AI Agent Checks
...
- User wants to set up `cfld check` in a project
...
cfld check --agent .continue/agents/security-review.md

# Run a Hub-published agent
cfld check --agent myorg/code-style

# Run multiple agents
cfld check --agent .continue/agents/security.md --agent .continue/agents/docs.md
```

### Compare against a specific base branch

```bash
cfld check --base develop
```
...
cfld check --format json

# Unified patch output (pipe to git apply)
cfld check --patch | git apply

# Stop on first failure
cfld check --fail-fast
```
...
```
cfld check  -  3 checks against main  -  5 changed files
...
| "No checks found"            | Create `.continue/agents/*.md` files or run `cfld login` for Hub checks                 |
...
| Patch conflicts with `--fix` | Apply patches manually: `cfld check --patch > changes.patch && git apply changes.patch` |
