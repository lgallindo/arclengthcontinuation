---
name: crisfield-check
description: Install and run the Arclength-Continuation CLI (`crisfield`) to execute AI agent checks on local code changes. Use when asked to "run checks", "lint with AI", "review my changes with crisfield", or set up Arclength-Continuation CI locally.
...
# crisfield check — Local AI Agent Checks
...
- User wants to set up `crisfield check` in a project
...
crisfield check --agent .continue/agents/security-review.md

# Run a Hub-published agent
crisfield check --agent myorg/code-style

# Run multiple agents
crisfield check --agent .continue/agents/security.md --agent .continue/agents/docs.md
```

### Compare against a specific base branch

```bash
crisfield check --base develop
```
...
crisfield check --format json

# Unified patch output (pipe to git apply)
crisfield check --patch | git apply

# Stop on first failure
crisfield check --fail-fast
```
...
```
crisfield check  -  3 checks against main  -  5 changed files
...
| "No checks found"            | Create `.continue/agents/*.md` files or run `crisfield login` for Hub checks                 |
...
| Patch conflicts with `--fix` | Apply patches manually: `crisfield check --patch > changes.patch && git apply changes.patch` |
