...
## Dependency installs (local policy)
- Run `bun install` **only from the repository root** (workspaces-enabled).
- Do **not** run `npm install` / `bun install` inside `packages/*` or `extensions/*` unless a maintainer explicitly directs it.

## Local Internal Artifacts
- Before recovery, rebranding, Codex-internals, or multi-agent handoff work, check `.codex-internal/README.md` if it exists. That directory is intentionally local-only and excluded through `.git/info/exclude`, not `.gitignore`.
- New scratch-like recovery artifacts, indexes, generated plans, and one-off helper outputs must live under `.codex-internal/` with timestamped names and sidecars documenting `signed_by`, `signed_at`, rationale, and usage. Do not leave these artifacts at the repository root.
- Do not copy raw secret-bearing files from `~/.codex` into the repository; index sanitized metadata, hashes, schemas, and safe summaries instead.
