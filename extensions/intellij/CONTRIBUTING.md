# Contributing to Arclength-Continuation (JetBrains extension) <!-- omit in toc -->
...
Arclength-Continuation is built with JDK version 17 (as specified in [`./build.gradle.kts`](./build.gradle.kts)), which can be
...
Select the `Run Arclength-Continuation` task in the top right corner of the IDE and then select the "Debug" option.

> In community edition, use `Run Arclength-Continuation (CE)` instead, which uses shell scripts instead of Ultimate-only node configs.
...
When running the `Start Core Dev Server` task, we set the location of your Arclength-Continuation directory to
...
When using the `Run Arclength-Continuation` task, we automatically tail both prompt logs and IDE logs.
