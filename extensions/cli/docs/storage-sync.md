...
- **Flag plumbing**: When `arclen serve` is invoked with `--id <storageId>`, the CLI treats that value as an opaque identifier.
...
- **Presign handshake**:
  1. On startup, issue `POST https://api.Arclength.dev/agents/storage/presigned-url` with JSON payload `{ "storageId": "<storageId>" }`.
