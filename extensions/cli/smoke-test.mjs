...
runTest("Bundle file exists", () => {
  ...
  if (!existsSync(resolve(__dirname, "dist/crisfield.js"))) {
    throw new Error("dist/crisfield.js not found");
  }
});

// Test 2: Check if wrapper script is executable
runTest("Wrapper script has shebang", () => {
  const content = readFileSync(resolve(__dirname, "dist/crisfield.js"), "utf8");
...
function getCLICommand(args = "") {
  ...
  if (isWindows) {
    return `node dist/crisfield.js ${args}`;
  } else {
    return `./dist/crisfield.js ${args}`;
  }
}
...
runTest("CLI works via npm link", () => {
  try {
    ...
    const output = execCommand("node dist/crisfield.js --version 2>&1");
    const packageJson = JSON.parse(
      readFileSync(resolve(__dirname, "package.json"), "utf8"),
    );
    if (!output.includes(packageJson.version)) {
