...
# E2E: headless cfld against a remote OpenAI-compatible llama-server.
...
# dist/cfld.js is the executable wrapper that invokes runCli(); dist/index.js
# is only the bundle module and does nothing when executed directly.
if [ ! -f dist/cfld.js ]; then
  echo "FAIL: dist/cfld.js missing — run npm run build first" >&2
  exit 1
fi
...
echo "E2E: running headless cfld"
output="$(FORCE_NO_TTY=true node dist/cfld.js -p --config "$workdir/config.yaml" \
  "Reply with exactly one word: PONG" 2>"$workdir/stderr.log")" || {
  echo "FAIL: cfld exited non-zero. stderr:" >&2
