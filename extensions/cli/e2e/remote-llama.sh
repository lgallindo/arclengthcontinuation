#!/usr/bin/env bash
# E2E: headless arclen against a remote OpenAI-compatible llama-server.
...
# dist/arclen.js is the executable wrapper that invokes runCli(); dist/index.js
# is only the bundle module and does nothing when executed directly.
if [ ! -f dist/arclen.js ]; then
  echo "FAIL: dist/arclen.js missing — run npm run build first" >&2
  exit 1
fi
...
echo "E2E: running headless arclen"
output="$(FORCE_NO_TTY=true node dist/arclen.js -p --config "$workdir/config.yaml" \
  "Reply with exactly one word: PONG" 2>"$workdir/stderr.log")" || {
  echo "FAIL: arclen exited non-zero. stderr:" >&2
