...
# --- 1. EDIT: cn modifies its own source ---------------------------------
edited=""
...
if [ -z "$edited" ]; then
  echo "FAIL: cn did not produce the requested edit in $ATTEMPTS attempts"
  cleanup
  exit 1
fi
...
echo "LOOP: EDIT OK (cn set version to $TARGET_VERSION)"
...
chmod +x extensions/cli/dist/cn.js
...
if [[ "$installed_version" != *"$TARGET_VERSION"* ]]; then
  echo "FAIL: reinstalled cn reports '$installed_version', expected $TARGET_VERSION"
  cleanup
  exit 1
fi
echo "LOOP: VERIFY OK (cn --version -> $installed_version)"

# --- 6. AUDIT: commit stays on the local branch for human review ------------
git add "$PKG"
git -c user.name="cn self-hosting loop" -c user.email="selfhost@localhost" \
  commit -q -m "selfhost-loop: cn set its own version to ${TARGET_VERSION}

Edit authored by headless cn (agentic tools) as part of the self-hosting
