  ...
  latestVersionCache = (async () => {
    try {
      ...
      const response = await fetch(
        `https://api.arclength-continuation.dev/cfld/info?id=${encodeURIComponent(id)}`,
        { signal },
      );
      if (!response.ok) {
      ...
      logger?.debug(
        "Warning: Could not fetch latest version from api.arclength-continuation.dev",
      );
      return null;
    }
  })();
...
getLatestVersion()
  ...
  .catch((error) => {
    logger?.debug(
      `Warning: Could not fetch latest version from api.arclength-continuation.dev: ${error}`,
    );
  });
