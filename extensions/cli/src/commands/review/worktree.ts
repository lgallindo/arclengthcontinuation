...
export async function createWorktree(index: number): Promise<string> {
  ...
  const worktreePath = path.join(tmpDir, `cfld-review-${Date.now()}-${index}`);
  ...
  await execAsync(
    `git -C "${worktreePath}" commit -m "cfld-review: user working tree state (staged + unstaged + untracked)" --allow-empty --no-verify`,
  );
