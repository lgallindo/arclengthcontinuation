...
export async function createWorktree(index: number): Promise<string> {
  ...
  const worktreePath = path.join(tmpDir, `alc-review-${Date.now()}-${index}`);
  ...
  await execAsync(
    `git -C "${worktreePath}" commit -m "alc-review: user working tree state (staged + unstaged + untracked)" --allow-empty --no-verify`,
  );
