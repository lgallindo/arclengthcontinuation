# Devbox entrypoint behavior (arclen serve)

Context: runloop resumes a devbox by re-running the same entrypoint script, which invokes `arclen serve --id <agentId> ...`. Because the entrypoint always replays, the CLI must avoid duplicating state on restart.
...
- **Environment persistence:** The devbox entrypoint (control-plane) writes all env vars to `~/.continue/devbox-env` and sources it before `arclen serve`, so keys survive suspend/resume. The CLI assumes env is already present.
