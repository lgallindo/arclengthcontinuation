...
export async function createSmokeContext(): Promise<SmokeTestContext> {
  const cliPath = path.resolve("dist/cfld.js");
  ...
  const testDir = await fs.mkdtemp(path.join(os.tmpdir(), "cfld-smoke-"));
...
/**
 * Runs `cfld` (headless) and returns stdout/stderr/exitCode.
 */
...
/**
 * Spawns `cfld serve` as a background subprocess and returns it along with
