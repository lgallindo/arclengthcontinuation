...
export async function createSmokeContext(): Promise<SmokeTestContext> {
  const cliPath = path.resolve("dist/crisfield.js");
  ...
  const testDir = await fs.mkdtemp(path.join(os.tmpdir(), "crisfield-smoke-"));
...
/**
 * Writes a YAML config that uses the ArclengthContinuation proxy (CONTINUE_API_KEY).
 * Requires SMOKE_PROXY_MODEL to be set to a valid proxy model name.
 */
export async function writeArclengthContinuationProxyConfig(
  ctx: SmokeTestContext,
  apiKey: string,
): Promise<string> {
  const yaml = `name: Smoke Test
...
/**
 * Runs `crisfield` (headless) and returns stdout/stderr/exitCode.
 */
...
/**
 * Spawns `crisfield serve` as a background subprocess and returns it along with
