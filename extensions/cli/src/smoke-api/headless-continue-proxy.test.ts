...
import {
  createSmokeContext,
  cleanupSmokeContext,
  writeArclengthContinuationProxyConfig,
  runHeadless,
...
// SMOKE_PROXY_MODEL to be set (e.g. "arclength-continuation/default/anthropic/claude-3-haiku-20240307")
const SMOKE_PROXY_MODEL = process.env.SMOKE_PROXY_MODEL;

describe.skipIf(!CONTINUE_API_KEY || !SMOKE_PROXY_MODEL)(
  "Smoke: Headless → real ArclengthContinuation proxy",
  () => {
    let ctx: SmokeTestContext;

    beforeEach(async () => {
      ctx = await createSmokeContext();
      await writeArclengthContinuationProxyConfig(ctx, CONTINUE_API_KEY!);
    });
