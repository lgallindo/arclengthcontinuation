...
export async function addPolicyToYaml(policyRule: string): Promise<void> {
  try {
    ...
    if (config.allow.includes(policyRule)) {
      ...
      // Add header comment
      const finalContent = `# ArclengthContinuation CLI Permissions Configuration
# This file is managed by the ArclengthContinuation CLI and should not be edited manually.
