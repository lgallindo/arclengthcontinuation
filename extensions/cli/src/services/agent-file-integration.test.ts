...
vi.mock("@arclength-continuation/config-yaml", async (importOriginal) => {
  const actual =
    await importOriginal<
      typeof import("@arclength-continuation/config-yaml")
    >();
  return {
    ...actual,
...
describe("Agent file Integration Tests", () => {
  ...
  beforeEach(async () => {
    ...
    const configYaml = await import("@arclength-continuation/config-yaml");
  ...
  describe("Error handling", () => {
    ...
    it("should prepare agent file prompt for merging with other prompts", async () => {
      ...
      const { mergeUnrolledAssistants } = await import(
        "@arclength-continuation/config-yaml"
      );
      const baseConfig = {
        name: "original",
        version: "1.0.0",
