...
The Arclength-Continuation CLI supports running in TTY-less environments (environments without a terminal/TTY), which is essential for:
...
const command = 'arclen -p "Analyze the current git diff"';
...
docker run --rm my-image arclen -p "Generate a README"
```
...
- name: Run Arclength-Continuation CLI
  run: |
    arclen -p "Review code changes" --format json
```
...
arclen -p "Generate commit message for current changes" --silent
```
...
Usage examples:
  arclen -p "please review my current git diff"
  echo "hello" | arclen -p
  arclen -p "analyze the code in src/"
  arclen -p --agent my-org/my-agent
```
...
```bash
arclen -p "your prompt" --config config.yaml
```
