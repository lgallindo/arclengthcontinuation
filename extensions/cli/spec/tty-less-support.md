...
const command = 'alc -p "Analyze the current git diff"';
...
docker run --rm my-image alc -p "Generate a README"
```
...
- name: Run Arclength-Continuation CLI
  run: |
    alc -p "Review code changes" --format json
```
...
alc -p "Generate commit message for current changes" --silent
```
...
Usage examples:
  alc -p "please review my current git diff"
  echo "hello" | alc -p
  alc -p "analyze the code in src/"
  alc -p --agent my-org/my-agent
```
...
```bash
alc -p "your prompt" --config config.yaml
```
