...
1. **CLI `--config` Flag** (highest priority)
   ...
   - Assistant slug (`owner/package`): Fetches from Arclength-Continuation platform
...
- Default assistant (`arclength-continuation/default-cli-config`)
...
- **Result**: Falls back to `arclength-continuation/default-cli-config`
