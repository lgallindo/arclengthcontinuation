...
export class VerticalDiffManager {
  ...
  clearForfileUri(fileUri: string | undefined, accept: boolean) {
    ...
    void vscode.commands.executeCommand(
      "setContext",
      "arclength-continuation.diffVisible",
      false,
    );
  ...
  async streamDiffLines(
    ...
    vscode.commands.executeCommand(
      "setContext",
      "arclength-continuation.streamingDiff",
      true,
    );
  ...
  async streamEdit({
    ...
    if (existingHandler) {
      ...
      // but extra delta was added for each line generated by ArclengthContinuation
      // Before adding this back, we need to distinguish between human and ArclengthContinuation
    ...
    try {
      ...
      if (abortController.signal.aborted) {
        void vscode.commands.executeCommand(
          "arclength-continuation.rejectDiff",
        );
      }
