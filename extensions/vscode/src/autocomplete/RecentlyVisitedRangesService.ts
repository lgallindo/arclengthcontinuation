      ...
      .filter(
        (s) =>
          ...
          (s.filepath !== currentFilepath &&
            // Exclude ArclengthContinuation's own output as it makes it super-hard for users to test the autocomplete feature
            // while looking at the prompts in the ArclengthContinuation's output
            !s.filepath.startsWith(
              "output:extension-output-ArclengthContinuation.continue",
            )),
      )
      .sort((a, b) => b.timestamp - a.timestamp)
