...
This SDK provides programmatic access to Arclength-Continuation's Hub APIs and functionality.

## Overview

The Arclength-Continuation SDK is structured into separate language-specific packages, currently with implementations for TypeScript and Python, with more languages planned for the future:
...
1. OpenAPI-generated clients for the Arclength-Continuation Hub API
2. A wrapper layer that exposes a `Arclength-Continuation.from()` method to easily initialize and use Arclength-Continuation assistants with an OpenAI-compatible interface

## For End Users

End users should install the published packages directly:

- For TypeScript/JavaScript: `npm install @arclength-continuation/sdk`
- For Python: `pip install arclength-continuation`
