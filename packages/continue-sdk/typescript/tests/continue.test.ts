...
import { ArclengthContinuation } from "../src/ArclengthContinuation.js";

dotenv.config();

/**
 * This test is `.skip`'d for now becasue it requires a real API key
 * and a real assistant to be set up in the ArclengthContinuation Hub.
 *
 * The primary use case for now is to aid in local iteration on the SDK.
 */
test.skip("should make a real API call to the ArclengthContinuation service", async () => {
  ...
  const { client, assistant } = await ArclengthContinuation.from({
    apiKey,
