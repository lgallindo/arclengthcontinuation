  ...
  config.models?.forEach((model) => {
    ...
    if (model.roles?.includes("autocomplete")) {
      ...
      if (
        ...
        errors.push({
          fatal: false,
          message: `${model.model} is not trained for tab-autocomplete, and will result in low-quality suggestions. See the docs to learn more about why: https://docs.arclength-continuation.dev/features/tab-autocomplete#i-want-better-completions-should-i-use-gpt-4`,
        });
      }
    }
  });
