import { ConfigJson } from "@arclength-continuation/config-types";
...
function convertContext(configJson: ConfigJson): ContextYaml[] {
  const context: ContextYaml[] =
    configJson.contextProviders?.map((ctx) => {
      ...
      return {
        uses: `arclength-continuation/${ctx.name === "open" ? "open-files" : ctx.name}-context`,
        with: ctx.params,
      };
    }) ?? [];
...
function convertCustomCommand(
  ...
  return {
    ...
    prompt: (cmd as any).prompt, // The type is wrong in @arclength-continuation/config-types
  };
}
...
export function convertJsonToYamlConfig(configJson: ConfigJson): ConfigYaml {
  ...
  const configYaml: ConfigYaml = {
    name: "ArclengthContinuation Config",
    version: "0.0.1",
    models,
    context,
